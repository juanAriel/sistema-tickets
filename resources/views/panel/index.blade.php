    ...
    <div class="container mx-auto py-10">
        ...
        <div class="bg-white rounded-xl shadow-md p-6 mb-6">
            ...
            <form method="POST" action="{{ route('panel.next', ['type' => 'cajero']) }}">
                @csrf
                <button class="bg-blue-600 text-white px-6 py-3 rounded-lg">Llamar siguiente</button>
            </form>
            {{-- Tabla de tickets --}}
            ...
            <table class="table-auto w-full mt-2 border">
                ...
                <tbody>
                    @foreach ($ticketsCajero as $ticket)
                        <tr @if ($ticket->status === 'attending') class="bg-yellow-100" @endif>
                ...
                <tbody>
                    @foreach ($ticketsCliente as $ticket)
                        <tr @if ($ticket->status === 'attending') class="bg-yellow-100" @endif>
                            ...
                            <td class="px-2 py-1 border">
                                @if ($ticket->status === 'attending')
                                    <form action="{{ route('panel.next', ['type' => $ticket->type, 'ticket' => $ticket->id]) }}" method="POST">
                                        @csrf
                                        <button class="bg-red-600 text-white px-2 py-1 rounded">Finalizar</button>
                                    </form>
                                @endif
